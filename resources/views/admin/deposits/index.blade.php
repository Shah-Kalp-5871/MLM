            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-[#1f1f1f]">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="px-6 py-4"><span class="text-slate-400">Manual ({{ $dep->method }})</span></td>
                        <td class="px-6 py-4 font-bold text-green-400">₹{{ number_format($dep->amount, 2) }}</td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="text-xs text-slate-300">{{ $dep->method }}</span>
                                <span class="text-[10px] text-slate-500 font-mono">{{ $dep->transaction_id }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($dep->proof_image)
                            <a href="{{ asset('storage/' . $dep->proof_image) }}" target="_blank" class="bg-purple-600/10 text-purple-400 hover:bg-purple-600/20 px-3 py-1 rounded-lg text-xs font-bold transition-all border border-purple-600/20">View Proof</a>
                            @else
