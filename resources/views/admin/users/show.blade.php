...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500">
            ...
            <span class="text-slate-200">User Profile #{{ $user->id }}</span>
        </div>
    ...
    <!-- Profile Header -->
    <div class="glass p-8 rounded-[2rem] border border-white/5 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
        <div class="absolute -right-20 -top-20 w-64 h-64 bg-purple-600/10 rounded-full blur-3xl"></div>
        <div class="w-24 h-24 rounded-3xl bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-[#1f1f1f] flex items-center justify-center text-4xl font-black text-slate-200 shadow-2xl">
        ...
        <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-slate-400 font-medium">{{ $user->email }} • {{ $user->phone }}</p>
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4">
                <span class="badge {{ $user->status == 'active' ? 'badge-success' : 'badge-danger' }} text-[10px] px-3 py-1 rounded-full uppercase font-black">{{ $user->status }} Member</span>
                <span class="bg-purple-600/10 text-purple-400 border border-purple-600/20 text-[10px] px-3 py-1 rounded-full uppercase font-black italic">Ref: {{ $user->referral_code }}</span>
                <span class="text-xs text-slate-500 font-bold uppercase tracking-tighter">Joined {{ $user->created_at->format('d M Y') }}</span>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4 w-full md:w-auto">
            ...
            <div class="glass p-4 rounded-2xl text-center border-[#1f1f1f]">
                ...
                <p class="text-xl font-black text-blue-400">{{ $user->referrals->count() }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="glass rounded-3xl overflow-hidden">
                <div class="p-6 border-b border-[#1f1f1f]">
                    <h3 class="font-bold tracking-tight">Direct Referrals</h3>
                </div>
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-[#1f1f1f]">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500 py-12">
        ...
        <div class="space-y-6">
            <div class="glass p-6 rounded-3xl space-y-6">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl glass border border-[#1f1f1f] flex items-center justify-center font-bold text-slate-500">U</div>
                    <div>
                        <p class="text-sm font-bold">{{ $user->upline->name ?? 'System Admin' }}</p>
                        <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{{ $user->upline_id ? 'Referrer' : 'Master Node' }}</p>
                    </div>
                </div>
            </div>
